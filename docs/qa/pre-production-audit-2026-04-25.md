...
| ~~AV-2~~ | ✅   | ~~Parse route exceeds 60s for >200-row statements; batches stranded in `parsing`~~ — **resolved 2026-04-28.** Parse pipeline moved to Inngest queue (`feat(AV-2)`, PR #8). Sync route enqueues an event and returns 202; the Inngest function runs the full pipeline with no time cap. Watchdog cron (`watchdog-stuck-imports`) registered to flip stranded batches to `failed`. Production cutover via `IMPORTS_ASYNC=true` env flag — sync path retained behind the flag for emergency rollback. Verified end-to-end via `__tests__/e2e/import-async-parse.spec.ts`.                                                                                                                                                               | n/a                                                                          | n/a — shipped                                                                                                | n/a        | n/a                                                                                        |
...
| Security        | 0        | 2 (SE-3,7)   | 2      | 2     | SE-1 ✅, SE-4 ✅, SE-6 ✅ |
| Availability    | 0        | 4            | 0      | 0     | AV-1 ✅, AV-2 ✅          |
| UX              | 0        | 2 (UX-1,3)   | 6      | 1     |                           |
