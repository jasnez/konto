...
| ~~AV-2~~ | ✅   | ~~Parse route exceeds 60s for >200-row statements; batches stranded in `parsing`.~~ — **descoped 2026-05-02 (PR #92, commit `7f56368`).** Inngest async pipeline (`feat(AV-2)`, PR #8) was shipped feature-flag OFF and never enabled in prod; cutover attempts hit Vercel Hobby `maxDuration` 60s cap on `step.run()` invocations, defeating the purpose. Removed entirely (`lib/inngest/`, `app/api/inngest/`, `inngest` dep, async branch, AV-2 cutover runbook). Sync path is the supported flow; `recoverStuckImports` (10-min user-load recovery) handles stranded `parsing` rows. If long-running parse becomes a real need: Vercel Pro ($20/mes for 300s `maxDuration`) is the cheaper next step than re-introducing Inngest.                                                                                                                                                                                                                                                                                | n/a                                                                          | n/a — descoped                                                                                               | n/a        | n/a                                                                                      |
