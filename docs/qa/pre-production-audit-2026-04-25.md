...
- **`00001_initial_schema.sql:127`** — `update_account_balance` trigger body is a passthrough no-op. `accounts.current_balance_cents` is never updated by the database. App layer compensates but any direct DB write (import RPC, seed, migrations) silently corrupts balances.
...
2. **Implement `update_account_balance` trigger** — balance correctness can no longer depend on developer discipline. M complexity, **high deployment risk** — requires balance backfill script and migration window.
...
**`buildUserExportJsonForRequest` — unbounded `transactions SELECT *` with category + merchant join, no LIMIT.** This is the query most likely to produce a 504 for any user with more than a few years of imported statements, and it produces zero partial output on timeout (all-or-nothing).
...
| ------- | ---------------------------------------------------------------------------------- | ----------- | ------ | -------- |
| DL-1    | `update_account_balance` trigger no-op; direct DB write corrupts balances silently | Med         | High   | **High** |
...
| AV-3    | Export non-streaming; OOM/timeout at ~100k transactions                    | Med         | High   | **High**     |
| AV-4    | FX loop sequential; multi-currency finalize ~30s                           | High        | Med    | **High**     |
| AV-5    | `count:exact` on every transactions page; >3s at ~100k rows                | Med         | Med    | **High**     |
| AV-6    | FX failure leaves batch stuck `importing` permanently                      | Med         | High   | **High**     |
...
| Risk ID | Description                                                                                     | Probability | Impact | Severity     |
| ------- | ----------------------------------------------------------------------------------------------- | ----------- | ------ | ------------ |
| MT-1    | `imports.ts` 1000 LoC, 5 concerns, no DI — largest SPOF                                         | High        | High   | **Critical** |
...
| AV-2    | 🔴   | Parse route exceeds 60s for >200-row statements; batches stranded in `parsing` | Largest real-world statements never finalize                                 | Move parse to background queue (Inngest/QStash); chunk by page; add watchdog cron at 90s                     | XL         | **Opus 4.7** — queue selection, state machine redesign, idempotency across async boundaries                 |
| MT-1    | 🔴   | `imports.ts` 1000 LoC, 5 concerns, no DI                                       | Every other import fix is high-risk and untestable                           | Split into `parse.ts`, `finalize.ts`, `dedupe.ts`, `fx.ts`, `persistence.ts`; inject FX + Gemini clients     | XL         | **Opus 4.7** — interface contract design + safe decomposition of complex interdependencies                  |
| DL-1    | 🔴   | `update_account_balance` trigger is a no-op                                    | Any direct DB write silently corrupts balances                               | Replace stub trigger in new migration; implement `AFTER INSERT/UPDATE/DELETE`; add backfill + integrity test | M          | **Opus 4.7** — SQL trigger correctness; soft-delete edge cases + multi-currency; mistake corrupts all users |
...
| SE-4    | 🔴   | No security headers                                                            | XSS, clickjacking, downgrade attacks viable                                  | Add `headers()` in `next.config.ts`; nonce-based CSP; ship in report-only 48h then enforce                   | S          | **Haiku 4.5** — config-only; directives fully specified in this report                                      |
...
| Risk ID | Tier | Problem                                              | Impact                                     | Fix approach                                                                            | Complexity | Model                                                                                   |
| ------- | ---- | ---------------------------------------------------- | ------------------------------------------ | --------------------------------------------------------------------------------------- | ---------- | --------------------------------------------------------------------------------------- |
| AV-3    | 🟠   | Export non-streaming                                 | OOM/timeout at ~100k transactions          | Stream via `ReadableStream` + cursor pagination in `app/api/export/data/route.ts`       | M          | **Sonnet 4.6** — streaming + cursor pattern, well-understood                            |
| AV-4    | 🟠   | FX loop sequential                                   | Finalize ~30s for multi-currency batches   | `Promise.all` over distinct (currency, date) pairs with in-memory cache                 | S          | **Haiku 4.5** — mechanical: `Promise.all` + `Map` cache, fully specified                |
| AV-5    | 🟠   | `count:exact` on every transactions page             | >3s at 100k rows                           | Switch to `count:estimated` + cursor pagination                                         | S          | **Sonnet 4.6** — pagination contract change touches multiple pages                      |
| AV-6    | 🟠   | FX failure leaves batch stuck `importing` forever    | Permanent dead-letter; no UI recovery      | Catch FX block, set `import_failed` with reason, add recovery RPC                       | S          | **Haiku 4.5** — targeted catch block + status update; scope fully defined               |
...
2. **DL-1** — Restores balance trigger to prevent silent corruption that no later fix can retroactively detect.
...
- DL-8 (RLS lint) → DL-1 (balance trigger) — trigger adds a new privileged write path; baseline lint should be clean first
...
| --- | ------------------------------------------------------------- | ----------------------------------------------------------- |
| 1   | DL-1 trigger draft + manual SQL review on staging             | SE-6 `logSafe()` + ESLint `no-console` rule                 |
| 2   | DL-1 finalize + backfill script; review agent's SE-6 PR       | DL-8 RLS lint rule + sweep redundant `eq('user_id')`        |
| 3   | DL-5 FX `numeric(20,10)` RPC contract + migration             | SE-4 CSP/HSTS/XFO/Referrer headers (report-only first)      |
...
2. **`update_account_balance` trigger + any `accounts.balance` writer**  
   Double-write or trigger recursion silently corrupts balances for all users.  
   **Gate:** balance-after-insert, balance-after-soft-delete, transfer-pair invariant, RLS isolation tests green before enabling.
...
| DL-1 (balance trigger)          | No (DB-level)             | No                                         | **Yes** — low-traffic window, backfill + reconciliation query before enable; rollback migration ready |
