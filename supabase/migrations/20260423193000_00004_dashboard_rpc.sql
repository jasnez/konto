-- =============================================================================
-- Dashboard monthly summary RPC
-- - One DB round-trip from app via supabase.rpc('get_monthly_summary', ...)
-- - CTE-based aggregation for current/previous month and account balance rollup
-- =============================================================================
...
begin
  ...
  if p_year = extract(year from current_date)::int
     and p_month = extract(month from current_date)::int then
    v_day_divisor := greatest(extract(day from current_date)::int, 1);
  else
